...
public class customerActions {
    ...
    private static void cancelTicketPurchase(Scanner scanner, Customer customer) {
        ...
        String totalPriceStr = (String) selectedPurchase.get("totalPrice");
        if (totalPriceStr != null && totalPriceStr.contains("$")) {
            totalPriceStr = totalPriceStr.replace("$", "");
            double refundAmount = Double.parseDouble(totalPriceStr);
        // Rest of your code for refund and cancellation
        } 
        else {
        // Handle the case when totalPriceStr is null or does not contain "$"
        System.out.println("Invalid total price format. Unable to calculate refund.");
        }
        double refundAmount = Double.parseDouble(totalPriceStr);        
